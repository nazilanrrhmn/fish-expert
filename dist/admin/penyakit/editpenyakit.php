...
if ($kdubah != "") {
  ...
  $edit_pencegahan = $data['pencegahan'];
  $gambar = $data['gambar'];
}
?>
<!DOCTYPE html>
...
<body>
  ...
  <div class="card col-md-6 mx-auto mt-5">
    <div class="card-body">
      ...
      <form method="post" action="./editpenyakit2.php" enctype="multipart/form-data">
        ...
        <div class="form-group">
          <label for="solusi">Pencegahan Penyakit :</label>
          <textarea class="form-control" rows="3" id="edit_pencegahan" name="edit_pencegahan"><?php echo $edit_pencegahan; ?></textarea>
        </div>
        <div class="form-group">
          <label for="solusi">Gambar Penyakit :</label>
          <input type="hidden" name="gambar_lama" value="<?= $gambar; ?>">
          <input type="file" name="filegambar" class="filegambar" id="filegambar" onchange="previewImage()">
          <img src="../../img/thumb/<?= $gambar ?>" width="120" style="display: block;" class="img-preview">
        </div>
