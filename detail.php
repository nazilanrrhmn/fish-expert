<?php
    ...
    include "koneksi.php";
    $query    =mysqli_query($koneksi, "SELECT * FROM tb_penyakit WHERE kdpenyakit='$kdpenyakit'");
...
<html lang="en">
  ...
  <body>
    ...
    <section id="detail" class="pt-36 pb-32">
      <div class="container">
      <h4 class="font-bold uppercase text-primary text-lg mb-3">
              DETAIL PENYAKIT
            </h4>
        <div class="flex flex-wrap">
          <div class="w-full px-4 flex justify-center items-center mb-10">
            <img src="./dist/img/thumb/<?= $result["gambar"]; ?>" width="30%" height="30%" alt="">
          </div>
          <div class="w-full px-4 flex justify-center items-center mb-2">
            <h2 class="font-bold text-dark text-3xl mb-5 max-w-md">
            <?= $result["nama_penyakit"]; ?>
            </h2>
          </div>
          <div class="w-full px-4 flex justify-center items-center mb-4">
            <p class="font-medium text-base text-secondary text-justify max-w-2xl lg:text-lg">
            <?= $result["definisi"]; ?>
            </p>
          </div>
        </div>
